...
async function loadCloud() { if(!isConfigured()){setSync('saved','Mahalliy saqlangan');return;} const {token,gist}=config(), localState=state;setSync('syncing','Yuklanmoqda...');try{const response=await fetch(`https://api.github.com/gists/${encodeURIComponent(gist)}`,{headers:{Authorization:`token ${token}`,Accept:'application/vnd.github+json'}});if(!response.ok)throw new Error(String(response.status));const gistData=await response.json(),file=gistData.files?.['activity-data.json'];if(!file)throw new Error('nofile');const cloud=safeParse(file.content);if(!cloud)throw new Error('json');state=normalizeState(cloud);const hasPendingToday=localStorage.getItem(PENDING_SYNC)===todayKey();if(hasPendingToday){const remoteIds=new Set((state.activities[todayKey()]||[]).map(item=>item.id));state.activities[todayKey()]=[...(state.activities[todayKey()]||[]),...(localState.activities[todayKey()]||[]).filter(item=>!remoteIds.has(item.id))];state.achievements=[...state.achievements,...localState.achievements.filter(item=>!state.achievements.some(found=>found.id===item.id))];}data=state.activities;saveLocal();render();setSync('saved','Saqlangan');if(hasPendingToday)syncCloud();}catch(error){handleSyncError(error);}}
...
async function syncCloud(settingsOnly=false) { /* Entries still PATCH only after today; tag colour preferences may sync independently. */ if(!isConfigured() || (!settingsOnly&&localStorage.getItem(PENDING_SYNC)!==todayKey()))return;const {token,gist}=config();setSync('syncing','Saqlanmoqda...');try{const response=await fetch(`https://api.github.com/gists/${encodeURIComponent(gist)}`,{method:'PATCH',headers:{Authorization:`token ${token}`,'Content-Type':'application/json',Accept:'application/vnd.github+json'},body:JSON.stringify({files:{'activity-data.json':{content:JSON.stringify(state,null,2)}}})});if(!response.ok)throw new Error(String(response.status));if(!settingsOnly)localStorage.removeItem(PENDING_SYNC);setSync('saved','Saqlangan');notify('GitHub Gist ga saqlandi.');}catch(error){handleSyncError(error);}}
...
function openSettings(){const current=config(),prefs=preferences();$('#tokenInput').value=current.token;$('#gistInput').value=current.gist;$('#goalInput').value=prefs.goal||'';$('#reduceMotionInput').checked=prefs.reduceMotion;$('#soundInput').checked=prefs.sound;$('#summariesInput').checked=prefs.summaries;renderTagColorSettings();openModal('settingsModal');}
...
$('#settingsForm').onsubmit=event=>{event.preventDefault();localStorage.setItem('gh_token',$('#tokenInput').value.trim());localStorage.setItem('gh_gist_id',$('#gistInput').value.trim());const goal=Number($('#goalInput').value);if(goal>0)localStorage.setItem('monthly_goal',String(goal));else localStorage.removeItem('monthly_goal');localStorage.setItem('reduce_motion',$('#reduceMotionInput').checked);localStorage.setItem('sound_enabled',$('#soundInput').checked);localStorage.setItem('summaries_enabled',$('#summariesInput').checked);document.querySelectorAll('[data-tag-color]').forEach(input=>state.tagColors[input.dataset.tagColor]=input.value);saveLocal();closeModal('settingsModal');render();syncCloud(true);};
